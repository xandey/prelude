...
%   $Revision: 1.1.6.40 $  $Date: 2011/07/05 19:02:08 $
...
if isUsingBuiltinEditor
    ...
    if ~isempty(err)
        ...
        try
            ...
            genericMessage = MException(errMessage.Identifier, errMessage.getString);
...
[status, result] = unix(['which ' strtok(applicationName) ]);
...
if status == 0
    ...
    if nargin == 1 || isempty(absPath)
      ...
      %command = [appInQuotes ' "' absPath '" &'];
        command = [applicationName ' "' absPath '" &'];
    end
